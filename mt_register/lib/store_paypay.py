...
def input_cash_log(browser, dt_now, store_info):
    try:
        ...
        if store_info.get("date"):
            ...
            date = dt.datetime.strptime(
                store_info["タイムスタンプ"], "%Y/%m/%d %H:%M:%S"
            )  # noqa
    ...
    try:
        ...
        target_element = browser.find_element(
            by=By.XPATH,
            value='//*[@class="ng-scope"]//*[contains(text(), "現金入力")]',  # noqa
        )
        ...
        # select date
        target_element = browser.find_element(
            by=By.CLASS_NAME, value="datepicker"
        )  # noqa
        target_element.click()
        sleep(0.5)
        ...
        for count in range(diff_month):
            target_element = browser.find_element(
                by=By.CLASS_NAME, value="picker__nav--prev"
            )
            target_element.click()
            sleep(0.5)
        ...
        target_element = browser.find_element(
            by=By.XPATH, value=f'//div[@aria-label="{date_for_xpath}"]'
        )
        target_element.click()
        sleep(0.5)
        ...
        if "入金" in store_info["type"]:
            target_element = browser.find_element(
                by=By.CSS_SELECTOR, value=".icon-minus-circle.ng-scope"
            )
            target_element.click()
            sleep(0.5)
        ...
        target_element = browser.find_element(
            by=By.XPATH, value='//input[@placeholder="入力してください"]'
        )
        target_element.clear()
        ...
        # カテゴリ選択ボタンクリック
        target_element = browser.find_element(
            by=By.XPATH, value='//*[@class="ng-scope"]//*[contains(text(), "未定")]'
        )
        target_element.click()
        sleep(0.5)
        ...
        # paypay はすべて小文字
        target_element = browser.find_element(
            by=By.XPATH, value='//input[@type="search"]'
        )
        target_element.clear()
        ...
        target_element = browser.find_element(
            by=By.XPATH,
            value=f'//*[@class="ng-scope"]//*[contains(text(), "{store_info["category"]}")]',
        )
        target_element.click()
        sleep(0.5)
...
def store(browser: selenium.webdriver.chrome.webdriver.WebDriver, regist_name):  # type: ignore  # noqa
    ...
    jsonf = join(
        os.path.abspath(join(file_path, "..")), f"GCP_Auth/{json_file_name}"
    )  # noqa
    ...
    header = [
        item.value for item in header
    ]  # ['タイムスタンプ', 'date', 'value', 'type', 'detail', 'category', 'state', 'store']
    ...
    for i in range(len(ss_data_2d)):
        ...
        try:
            # 元データstr -> intに書き換え
            ss_data_2d[i][col_num_value].value = int(
                ss_data_2d[i][col_num_value].value
            )  # noqa
